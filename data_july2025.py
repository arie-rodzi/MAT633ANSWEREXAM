...
$$\\text{Very } A \\text{ and somewhat } B = \\frac{0.1008}{x_1} + \\frac{0.4116}{x_2} + \\frac{0.0495}{x_3} + \\frac{0.4544}{x_4}$$"""
    },
    {
        ...
        "topic": "Algebraic Sum Hedge Applications",
        "question": """Using the same definitions provided in Question 2(b)(i), determine the fuzzy set for: **Somewhat not $A$ or not somewhat $B$**.""",
        "answer": """**SUGGESTED ANSWER:**
...
$$\\mu_{\\tilde{X} \\to Y}(x,y) = \\frac{0.5}{(x_1,y_1)} + \\frac{0.6}{(x_1,y_2)} + \\frac{1}{(x_1,y_3)} + \\frac{1}{(x_2,y_1)} + \\frac{1}{(x_2,y_2)} + \\frac{1}{(x_2,y_3)} + \\frac{1}{(x_3,y_1)} + \\frac{1}{(x_3,y_2)} + \\frac{1}{(x_3,y_3)}$$"""
    },
    {
        ...
        "question": """Suppose $C$ and $D$ are discrete fuzzy sets defined on $C = \\{c_1, c_2\\}$ and $D = \\{d_1, d_2, d_3\\}$, respectively. Let the fuzzy implication relation $C \\to D$ be configured as follows:
...
$$\\tilde{C} = \\frac{0.2}{c_1} + \\frac{0.33}{c_2}$$"""
    },
    {
        ...
        "question": """Let fuzzy sets 'strong' and 'stable' be defined on continuous universes $x \\in [0,100]$ and $y \\in [0,10]$ respectively, where:
...
Using the Dienes-Rescher implication $\\mu_{Q_D}(x, y) = \\max(1 - \\mu_{FP_1}(x), \\mu_{FP_2}(y))$, construct the structural fuzzy relation of the following linguistic rule: **"IF the economy is very strong, THEN the Ringgit is stable"**.""",
        "answer": """**SUGGESTED ANSWER:**
...
* **Step 2: Construct the Dienes-Rescher relation profile via piecewise synthesis**
  ...
  \\end{cases}$$"""
    },
    {
        ...
        "question": """Let $A$, $B$, and $C$ be continuous fuzzy sets defined on $x \\in [0,8]$, $y \\in [0,10]$, and $z \\in [0,10]$ respectively, where:
